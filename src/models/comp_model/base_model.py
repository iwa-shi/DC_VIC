...
class BaseModel(nn.Module):
    ...
    def img_preprocess(self, real_images: Tensor, is_train: bool=True) -> Tensor:
        out = real_images
    ...
    def img_postprocess(self, *images:Tensor, size: Tuple[int, int], is_train: bool) -> Union[Tensor, Tuple[Tensor, ...]]:
        H, W = size
        out = []
        for img in images:
    ...
    def data_preprocess(self, is_train: bool=True, **input_data) -> Dict:
        raise NotImplementedError()
    
    def data_postprocess(self, input_data: Dict, out_dict: Dict, is_train: bool=True) -> Dict:
        raise NotImplementedError()

    def run_model(self, is_train: bool=True, **input) -> Dict:
        processed_input = self.data_preprocess(**input, is_train=is_train)
        out_dict = self.forward(**processed_input, is_train=is_train)
        return self.data_postprocess(processed_input, out_dict, is_train=is_train)

    def forward(self, is_train: bool=True, **kwargs):
        raise NotImplementedError()
    ...
    def load_learned_weight(self, ckpt_path: str, strict: bool=False) -> None:
        ...
        if not strict:
            ...
            out = self.load_state_dict(new_state_dict)
    ...
    def pad_images(self, *images:torch.Tensor) -> Union[torch.Tensor, Tuple[torch.Tensor, ...]]:
        out = []
        for img in images:
    ...
    @staticmethod
    def _pad_image(x: torch.Tensor, stride: int, mode: str='reflect') -> torch.Tensor:
        _, _, H, W = x.size()
    ...
    def crop_images(self, *images:torch.Tensor, size: Tuple[int, int]) -> Union[torch.Tensor, Tuple[torch.Tensor, ...]]:
    ...
    @staticmethod
    def _crop_image(x: torch.Tensor, H: int, W: int) -> torch.Tensor:
        """
